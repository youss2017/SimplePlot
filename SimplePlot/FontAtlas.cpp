...
static std::optional<vector<uint8_t>> load_file(const char* filename) {
	...
	size = fread(buffer.data(), 1, size, file);
	fclose(file);
	buffer.resize(size);
...
FontAtlas& splot::FontAtlas::LoadTTFont(const std::string& file)
{
	m_TTFile = load_file(file.c_str());
	if (!m_TTFile) {
...
void splot::FontAtlas::BuildAtlas(float fontSize, bool sdf, bool multithreaded)
{
...
std::vector<uint8_t> splot::FontAtlas::_GenerateCodepoint(float fontSize, wchar_t ch, uint32_t* pWidth, uint32_t* pHeight)
{
	int w = 0, h = 0;
...
std::vector<uint8_t> splot::FontAtlas::_GenerateCodepointAligned(float fontSize, wchar_t ch, uint32_t* pWidth, uint32_t* pHeight, uint8_t alignment)
{
	int w = 0, h = 0;
	auto bmp_raw = stbtt_GetCodepointBitmap(&m_StbFontInfoStructure, 0, stbtt_ScaleForPixelHeight(&m_StbFontInfoStructure, fontSize), ch, &w, &h, 0, 0);
...
std::vector<uint8_t> splot::FontAtlas::_GenerateSdfCodepoint(float fontSize, wchar_t ch, uint32_t* pWidth, uint32_t* pHeight, uint32_t targetResolution)
{
	// develop sdf
...
std::vector<FontAtlas::QuadVertex> splot::FontAtlas::GenerateTextMesh(const std::wstring& text, int x, int y, int screenWidth, int screenHeight, float pixelSize)
{
